...
export interface BLConsoleEvent extends BLEvent {
    type: "console"
    name: 'console-assert' |
        'console-clear' |
        'console-count' |
...
export type BLGlobalErrorEvent =
    BLEvent
    & { message: string, source: string, lineno: string, colno: string, error: string, filename: string, stack: string }
export type BLGlobalPromiseErrorEvent =
    BLEvent
    & { reason: string, filename: string, stack: string }
...
export type BLCssRuleAddEvent = BLEvent & { index: number, rule: string, target: StyleSheet["ownerNode"] }
export type BLCssRuleRemoveEvent = BLEvent & { index: number, target: StyleSheet["ownerNode"] }
export type BLPerformanceCpuEvent = BLEvent & { performance: number }
export type BLPerformanceMemoryEvent =
    BLEvent
...
export interface BLMoveEvent extends BLEvent {
...
export interface BLMouseEvent extends BLEvent {
    x: number;
    y: number;
    ...
    target,
    currentTarget?,
    name: "touchmove" | "mousemove" | "mouseup" | "mousedown" | "click" | "contextmenu" | "dblclick" | "touchstart" | "touchend"
}

export interface BLScrollEvent extends BLEvent {
    x: number;
    y: number;
    target,
    currentTarget?,
